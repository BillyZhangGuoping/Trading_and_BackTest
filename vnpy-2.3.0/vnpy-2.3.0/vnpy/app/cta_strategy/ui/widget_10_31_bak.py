...
class CtaManager(QtWidgets.QWidget):
	...
	def init_ui(self):
		...
		NewVQbox1.addWidget(self.cta_trade_monitor, QtCore.Qt.AlignTop)
...
class NewSampleDataMonitor(QtWidgets.QTableWidget):
	...
	def update_data(self, data: dict):
		...
		if self.cells["pos"]:
			if int(self.cells['pos'].text()) > 0:
				self.cells["pos"].setForeground(QtGui.QColor("red"))
			elif int(self.cells['pos'].text()) < 0:
	...
	def StrategyManagerChoose(self):
		...
		if local_strategy.bg.last_tick:
