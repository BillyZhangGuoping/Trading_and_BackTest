...
class OneMAMACDBarCloseStrategy(CtaTemplate):
    """"""
    ...
    def on_bar(self, bar: BarData):
        ...
        if self.pos > 0:
            ...
            self.cancel_all()
            if max(macd[-3:])< -self.BAR_LIMIT:
                self.sell(bar.close_price, True, lock=False, net=True)
            else:
            ...
            self.cancel_all()
            if min(macd[-3:])> self.BAR_LIMIT:
                self.cover(bar.close_price, True, lock=False, net=True)
            else:
