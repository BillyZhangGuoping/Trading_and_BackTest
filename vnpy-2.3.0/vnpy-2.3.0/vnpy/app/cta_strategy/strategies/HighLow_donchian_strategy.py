...
class HighLowDochianStrategy(CtaTemplate):
    """"""
    author = "Billy"
    init_pos = 0
    ...
    parameters = ["fixed_size", "bar_time", "bar_array_length", "Markup", "price_up", "price_down", "price_cut", "dochian_window", "boll_dev", "isAG",
                  "lose_count_number","deal_open_count_number","std_limitation","tick_touch_limit"]
    variables = ["real_Markup", "std", "trade_price", "profit_price", "close_long_price", "close_short_price", "lose_count", "deal_open_count"]

    def __init__(self, cta_engine, strategy_name, vt_symbol, vt_local, setting):
        """"""
        super(HighLowDochianStrategy, self).__init__(
            cta_engine, strategy_name, vt_symbol, vt_local, setting
        )

        self.start_timePM = time(hour=9, minute=13)
        self.start_timeAM = time(hour=13, minute=42)
    ...
    def on_start(self):
        ...
        self.real_price_cut = self.price_cut * self.contact.pricetick
    ...
    def on_tick(self, tick: TickData):
        ...
        if self.pos == 0 and self.lose_count <=0:
            ...
            if tick.last_price < self.profit_price:
                ...
                self.close_short_price = self.profit_price + self.real_price_cut
            ...
            if tick.last_price > self.profit_price:
                self.profit_price = tick.last_price
                self.close_long_price = self.profit_price - self.real_price_cut
    ...
    def on_min_bar(self, bar: BarData):
        ...
        if not am.inited:
            return
        self.std = am.std(self.dochian_window)
        ...
        if self.check_trading_time(bar) and self.trading:
            ...
            if self.pos == 0 and self.lose_count <= 0:
                ...
                if self.std < self.std_limitation:
                    self.high, self.low = am.donchian(self.dochian_window)
                    self.high = self.high + self.real_Markup
                    self.low = self.low - self.real_Markup
    ...
    def on_trade(self, trade: TradeData):
        ...
        if trade.offset == Offset.OPEN:
            ...
            if trade.direction == Direction.LONG and self.trade_price < self.close_short_price:
                self.lose_count = self.lose_count_number
            elif trade.direction == Direction.SHORT and self.trade_price > self.close_long_price:
                self.lose_count == self.lose_count_number
