...
class BarSTDJumpTrendDownStrategy(CtaTemplate):
    """"""
    ...
    def on_time_bar(self, bar: BarData):
        ...
        if self.trading:
            self.write_log(
                f" jump : {self.jump}, Touch_JUMP : {self.Touch_JUMP}, self.open_count_down: {self.open_count_down},turn_count:{self.turn_count}, touch_open_price :{self.touch_open_price}, jump_jump_price: {self.jump_jump_price}")
        else:
            std = am.std(self.STD_WINDOWS)
            self.jump = std*self.STD_MULTPLE
            self.mark_up = std*self.STD_MARK_UP
            self.mark_down = std*self.STD_MARK_DOWN
    ...
    def on_trade(self, trade: TradeData):
        ...
        if trade.offset == Offset.OPEN:
            ...
            if trade.direction == Direction.LONG and self.entry_price < trade.price:
                self.trade_close = 1
            elif trade.direction == Direction.SHORT and self.entry_price > trade.price:
                self.trade_close = 1
