...
class OneMAMACDRatioBarStrategy(CtaTemplate):
    """"""
    ...
    triger_profit_1 = 15.0
    triger_profit_2 = 25.0
    triger_profit_3 = 35.0
    triger_profit_4 = 45.0
    ...
    def on_bar(self, bar: BarData):
        ...
        if self.pos == 0:
            ...
            self.price_rate = 0
            self.cancel_all()
            if (bar.close_price - MA_value_list[-2])*100.0/(MA_value_list[-2]*self.margin_ratio) >= self.start_rate and round(MA_value_list[-1],1) > round(MA_value_list[-2],1) and round(hist[-1],1)> round(hist[-2],1)  > round(hist[-3],1) and self.MACD_BAR > self.BAR_LIMIT \
            ...
            elif (bar.close_price - MA_value_list[-2])*100.0/(MA_value_list[-2]*self.margin_ratio) <= -self.start_rate and round(MA_value_list[-1],1) < round(MA_value_list[-2],1) and round(hist[-1],1)< round(hist[-2],1) < round(hist[-3],1)  and self.MACD_BAR < -self.BAR_LIMIT \
                    and self.close_bar_count >= self.CLOSE_BAR and self.new_time_bar == True:
        ...
        if self.pos > 0:
            ...
            self.price_rate = (self.intra_trade_high - self.entry_price) * 100.0 / (self.entry_price*self.margin_ratio)
            ...
            self.price_rate = (self.entry_price - self.intra_trade_low) * 100.0 / (self.entry_price*self.margin_ratio)
