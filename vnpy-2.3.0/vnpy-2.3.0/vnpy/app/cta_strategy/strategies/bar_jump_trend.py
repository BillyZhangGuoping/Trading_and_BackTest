...
class BarJumpTrendDownStrategy(CtaTemplate):
    ...
    def on_time_bar(self, bar: BarData):
        ...
        if self.open_count_down <= 0:
            ...
            if not self.not_trading_time:
                if bar.close_price - self.last_close_price > self.jump:
                    actul_jump = min(bar.high_price - self.last_close_price,4*self.jump)
                    self.Touch_JUMP = 1
                    self.turn_count = 0
                    ...
                    actul_jump = min(self.last_close_price - bar.low_price,4*self.jump)
                    self.Touch_JUMP = -1
                    self.turn_count = 0
                    self.open_count_down = self.OPEN_WINDOWS
        ...
        if self.trading:
            self.write_log(
                f"last_close_price: {self.last_close_price}, bar.close: {bar.close_price} Touch_JUMP : {self.Touch_JUMP}, not_trading_time : {self.not_trading_time}, self.open_count_down: {self.open_count_down},turn_count:{self.turn_count}, price_before_jump: {self.price_before_jump},  jump_jump_price: {self.jump_jump_price}")
