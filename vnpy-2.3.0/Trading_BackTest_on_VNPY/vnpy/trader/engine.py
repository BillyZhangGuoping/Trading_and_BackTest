...
class EmailEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.main_engine.send_email_attchment = self.send_email_attchment
    ...
    def send_email_attchment(self, subject: str, content: str, receiver: str = "",attch_dir = "",filename = "") -> None:
        """"""
        # Start email engine when sending first email.
        if not self.active:
            self.start()
...
class exportCSVEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.main_engine.add_method_sched = self.add_method_sched
    ...
    def add_method_sched(self,method):
        self.method_list.append(method)

    def register_time_event(self,event: Event):
        self.count += 1
        if self.count > 100 and self.notExportAlready:
            self.count = 0
            if time(18,0) > datetime.now().time() > time(0,11) and 6 > datetime.now().weekday() > 0:
                pass
                # pass
                # self.batchExport(1)
    ...
    def batchExport(self,days = 1,path = None):
        ...
        self.exportCSV(self.widgetsDict.get("account", None),"account")
        self.exportCSV(self.widgetsDict.get("position", None),"position")
        cta_engine = self.main_engine.get_engine("CtaStrategy")

        if cta_engine:
            end_date = datetime.now()
            start_date = datetime.now() - timedelta(days =1)
            cta_engine.transprot_all_triggered_strategies_order(start_date,end_date)
