...
class BacktesterManager(QtWidgets.QWidget):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self.main_engine.add_method_sched(self.download_all_symbol)
    ...
    def init_ui(self):
        ...
        # self.symbol_line = QtWidgets.QComboBox()
        ...
        self.margin_line = QtWidgets.QLineEdit("0.000025")
        self.rate_type = QtWidgets.QComboBox()
        self.rate_type.addItems(["按比率", "按手数"])
    ...
    def load_backtesting_setting(self):
        ...
        # self.vt_symbol_list = setting["symbol_list"]
        # self.vt_symbol_list_for_down = []
        #
        # for symbol in self.vt_symbol_list:
        #     self.symbol_line.addItem(symbol["vt_symbol"])
        #     self.vt_symbol_list_for_down.append(symbol["vt_symbol"])
        #
        # self.symbol_line.activated[int].connect(self.symbol_change)
        #
        # self.symbol_line.setCurrentIndex(
        #     self.symbol_line.findText(self.vt_symbol_list[0]["vt_symbol"])
        # )
        # self.rate_line.setText(str(self.vt_symbol_list[0]["rate"]))
        #
        # self.rate_type.setCurrentIndex(
        #     self.rate_type.findText(self.vt_symbol_list[0]["rate_type"])
        # )
        # self.margin_line.setText(str(self.vt_symbol_list[0]["marginRate"]))
        # self.slippage_line.setText(str(self.vt_symbol_list[0]["slippage"]))
        # self.size_line.setText(str(self.vt_symbol_list[0]["size"]))
        # self.pricetick_line.setText(str(self.vt_symbol_list[0]["pricetick"]))
        # self.capital_line.setText(str(self.vt_symbol_list[0]["capital"]))

    def symbol_change(self,symbol_int):
        # print(symbol_int)
        self.symbol_line.setCurrentIndex(
            self.symbol_line.findText(self.vt_symbol_list[symbol_int]["vt_symbol"])
        )
        self.rate_line.setText(str(self.vt_symbol_list[symbol_int]["rate"]))

        self.rate_type.setCurrentIndex(
            self.rate_type.findText(self.vt_symbol_list[symbol_int]["rate_type"])
        )
        self.margin_line.setText(str(self.vt_symbol_list[0]["marginRate"]))
        self.slippage_line.setText(str(self.vt_symbol_list[symbol_int]["slippage"]))
        self.size_line.setText(str(self.vt_symbol_list[symbol_int]["size"]))
        self.pricetick_line.setText(str(self.vt_symbol_list[symbol_int]["pricetick"]))
        self.capital_line.setText(str(self.vt_symbol_list[symbol_int]["capital"]))
    ...
    def process_optimization_finished_event(self, event: Event):
        ...
        self.save_and_send_csv_more()

    def save_and_send_csv_more(self) -> None:
        """
        ...
        with open(path, "w", encoding='utf-8-sig') as f:
            ...
            header_list = list(self.backtestSetting.keys())
            result_values = self.backtester_engine.get_result_values()

            dictResult = eval(result_values[0][0])

            dictResult.pop("init_pos", 0)
            dictResult.pop("init_entry_price", 0)
            dictResult.pop("fixed_size", 0)

            header_list.extend(list(dictResult.keys()))
            header_list.extend(result_values[0][2])
            writer.writerow(header_list)
            for tp in result_values:
                setting, target_value, parameter = tp
                row_data = list(self.backtestSetting.values())
                dictResult2 = eval(setting)
                dictResult2.pop("init_pos", 0)
                dictResult2.pop("init_entry_price", 0)
                dictResult2.pop("fixed_size", 0)
                row_data.extend(list(dictResult2.values()))
                row_data.extend(parameter.values())
                writer.writerow(row_data)

        # (self, subject: str, content: str, receiver: str = "", attch_dir = "", filename = "")
        self.main_engine.send_email_attchment(subject = Optimization_summary, content = Optimization_summary, attch_dir = filepath, filename = filename )

    def start_backtesting(self,local_vt_symbol = None):
        """"""
    ...
    def start_optimization(self):
        ...
        rate = float(self.rate_line.text())
        rate_type = self.rate_type.currentText()
        margin_rate = self.margin_line.text()
        slippage = float(self.slippage_line.text())
        size = float(self.size_line.text())
        pricetick = float(self.pricetick_line.text())
        capital = float(self.capital_line.text())
        ...
        self.backtestSetting =  {
            ...
            "策略名": class_name,
            "品种": vt_symbol,
            "测试开始时间": start.strftime('%Y-%m-%d'),
            "测试结束时间" : end.strftime('%Y-%m-%d'),
            "手续费率": rate,
            "手续费类型": rate_type,
            "保证金率": margin_rate,
            "交易滑点": slippage,
    ...
    def download_all_symbol(self):
        start = datetime(2012,10,10)
        start = DB_TZ.localize(start)
        end = datetime.now()
        end = DB_TZ.localize(end)
        while self.vt_symbol_list_for_down:
            if self.backtester_engine.thread:
                self.write_log("已有任务在运行中，请等待完成")
                return False

            self.backtester_engine.start_downloading(
                self.vt_symbol_list_for_down.pop(),
                "1m",
                start,
                end
            )
...
class OptimizationResultMonitor(QtWidgets.QDialog):
    ...
    def __init__(
        self, result_values: list, target_display: str, backtestSetting: dict
    ):
        """"""
        super().__init__()
        ...
        self.backtestSetting = copy(backtestSetting)
    ...
    def init_ui(self):
        ...
        button.clicked.connect(self.save_csv_more)
    ...
    def save_csv_more(self) -> None:
        ...
        with open(path, "w", encoding='utf-8-sig') as f:
            ...
            dictResult = eval(self.result_values[0][0])
            ...
            header_list.extend(list(dictResult.keys()))
            header_list.extend(self.result_values[0][2])
            writer.writerow(header_list)
            for tp in self.result_values:
                setting, target_value, parameter = tp
                row_data = list(self.backtestSetting.values())
