...
class SpreadAlgoTemplate:
    ...
    def calculate_traded_price(self):
        ...
        if not isinstance(spread, AdvancedSpreadData):
            ...
            self.traded_price = round_to(self.traded_price, pricetick)
        # Advanced spread
        else:
            data = {}
...
class SpreadStrategyTemplate:
    ...
    author: str = ""
    SymbolSizeFile = "SymbolSize.json"
    parameters: List[str] = []
    variables: List[str] = []
    ...
    def getSymbolSize(self):
		...
		加载配置路径
		"""
        setting = load_json(self.SymbolSizeFile)
        return setting[self.spread.active_leg.vt_symbol[:2]]["pricetick"]
