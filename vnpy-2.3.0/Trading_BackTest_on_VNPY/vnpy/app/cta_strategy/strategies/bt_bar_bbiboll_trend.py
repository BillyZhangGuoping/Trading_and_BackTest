...
class BT_BarBBIBOLLStrategy(CtaTemplate):
    """"""
    ...
    def on_time_bar(self, bar: BarData):
        ...
        if self.open_count_down <= 0:
            if self.pos == 0:
                self.cancel_all()
            self.Touch_JUMP = 0
            self.bbi_high, self.mid, self.bbi_low = am.bbiboll(self.BBI_WINDOWS, self.BBI_DEV)
            # self.diff = (self.bbi_high - self.bbi_mid)
            if bar.close_price >= self.bbi_high:
                self.Touch_JUMP = 1
                self.turn_count = 0
                self.open_count_down = self.OPEN_WINDOWS
                self.jump_price = bar.close_price
                self.price_before_jump = self.bbi_mid
            elif bar.close_price  <= self.bbi_low:
                self.Touch_JUMP = -1
                self.turn_count = 0
                self.jump_price = bar.close_price
                self.price_before_jump = self.bbi_mid
            ...
            if self.pos == 0 and not self.not_trading_time:
                ...
                if self.Touch_JUMP == 1:
                    ...
                    elif bar.close_price < self.price_before_jump:
