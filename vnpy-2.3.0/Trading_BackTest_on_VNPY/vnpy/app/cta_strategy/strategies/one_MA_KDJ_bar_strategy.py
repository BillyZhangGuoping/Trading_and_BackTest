...
class OneMAKDJBarStrategy(CtaTemplate):
    """"""
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, vt_local, setting):
        ...
        self.len_list = self.OPEN_KDJ_COUNT
        self.MA_Ratio = self.MA_Ratio  * 0.0001

        self.KDJ_bar_list = np.zeros(self.len_list)
    ...
    def on_bar(self, bar: BarData):
        ...
        local_high_array = copy(self.am.high_array)
        local_high_array[:-1] = local_high_array[1:]
        local_high_array[-1] = bar.high_price


        local_low_array = copy(self.am.low_array)
        local_low_array[:-1] = local_low_array[1:]
        local_low_array[-1] = bar.low_price

        MA_value_list = talib.SMA(local_close_array, self.MA_Windows)
        self.MA_value = MA_value_list[-2]
        slowk_matype = 0
        K,D = talib.STOCH( local_high_array, local_low_array, local_close_array, self.fastk_period, self.slowk_period, slowk_matype, self.slowd_period,slowk_matype)
        result = K - D

        self.KDJ_J_BAR = result[-1]
        ...
        self.KDJ_bar_list = [1 if v > self.BAR_LIMIT else -1 if v < -self.BAR_LIMIT else 0 for v in result[- self.len_list:]]
        if self.pos == 0:
            ...
            self.price_rate = 0
            self.cancel_all()
            if bar.close_price > MA_value_list[-1] and MA_value_list[-1] > MA_value_list[-3] and abs(MA_value_list[-1] -MA_value_list[-4])/MA_value_list[-1] > self.MA_Ratio and sum(
                    self.KDJ_bar_list[
                    -self.OPEN_KDJ_COUNT:]) >= self.OPEN_KDJ_COUNT  and self.close_bar_count >= self.CLOSE_BAR and self.new_time_bar == True:
                self.buy(bar.close_price+1, self.fixed_size, True)
            elif bar.close_price < MA_value_list[-1] and MA_value_list[-1] < MA_value_list[-3] and abs(MA_value_list[-1] -MA_value_list[-4])/MA_value_list[-4] > self.MA_Ratio and sum(
                    self.KDJ_bar_list[
                    -self.OPEN_KDJ_COUNT:]) <= -self.OPEN_KDJ_COUNT  and self.close_bar_count >= self.CLOSE_BAR and self.new_time_bar == True:
    ...
    def on_time_bar(self, bar: BarData):
        ...
        self.KDJ_J_BAR = am.kdj(self.fastk_period, self.slowk_period, self.slowk_period, False)
        self.put_event()
