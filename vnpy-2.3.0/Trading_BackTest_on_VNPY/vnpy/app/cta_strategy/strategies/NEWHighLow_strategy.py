...
class NEWHighLowStrategy(CtaTemplate):
    """"""
    author = "Billy"
    init_pos = 0
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, vt_local, setting):
        """"""
        super(NEWHighLowStrategy, self).__init__(
    ...
    def on_tick(self, tick: TickData):
        ...
        if self.pos == 0 and self.start:
            ...
            if self.tick_touch <= 0:
                if self.touch_start_high_point and self.old_bar_pass ==1:
                    self.send_open_short_deal(self.high-self.contact.pricetick)
                    self.start = False
                elif self.touch_start_low_point and self.old_bar_pass == -1:
                    self.send_open_long_deal(self.low+self.contact.pricetick)
                    self.start = False
        elif self.pos < 0:
    ...
    def on_min_bar(self, bar: BarData):
        ...
        if bar.high_price >=  self.high:
            self.old_bar_pass = 1
        elif bar.low_price <= self.low:
            self.old_bar_pass = -1
        else:
            self.old_bar_pass = 0
        ...
        if self.check_trading_time(bar) and self.trading:
            ...
            if self.pos == 0 and self.lose_count <= 0:
                ...
                if self.high <= self.boll_up and self.low >= self.boll_down and self.std <self.std_limitation:
                    self.start = True
