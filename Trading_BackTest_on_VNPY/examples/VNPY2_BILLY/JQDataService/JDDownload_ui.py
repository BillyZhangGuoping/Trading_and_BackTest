...
class DownloadManager(QtWidgets.QWidget):
    ...
    def tab_high_low_checkUI(self):
        ...
        self.new_button = QtWidgets.QPushButton("新增极值")
        self.min_max_button.setFixedHeight(self.min_max_button .sizeHint().height() * 2)
        self.new_button.setFixedHeight(self.new_button.sizeHint().height() * 2)
        self.min_max_button.clicked.connect(self.min_max_check)
        self.new_button.clicked.connect(self.new_check)
    ...
    def min_max_check(self):
        ...
        compareResult = self.downSevice.compareMaxMin(max_days, past_day)
        print(compareResult)

    def new_check(self):
        max_days = self.max_days_spin.value()
        past_day = self.past_day_spin.value()
        compareResult = self.downSevice.new_MaxMin(max_days, past_day)
        print(compareResult)
    ...
    def save_csv_more(self):
        ...
        with open(path, "w", encoding='utf-8-sig') as f:
            self.downloadData.to_excel(path,index=False)
