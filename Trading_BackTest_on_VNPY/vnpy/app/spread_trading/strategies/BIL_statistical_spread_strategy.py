...
class BILStatisticalSpreadStrategy(SpreadStrategyTemplate):
    """"""
    ...
    def on_spread_data(self):
        ...
        if not ((self.update_time > self.start_timePM and self.update_time <self.exit_timeAM) or (self.update_time > self.start_timeNI and self.update_time <self.exit_timeNI)):
        ...
        # Long position
            ...
            if not self.sell_algoid:
                if self.PosPrice != 0.0:
                    self.sell_price = self.PosPrice + (self.check_Ratio+1)*self.localPriceTick
        ...
        # Short position
            ...
            if not self.cover_algoid:
                if self.PosPrice != 0.0:
                    self.cover_price = self.PosPrice - (self.check_Ratio+1)*self.localPriceTick
    ...
    def on_spread_bar(self, bar: BarData):
        ...
        if self.spread_pos == 0.0:
            self.boll_mid = stats.mode(self.am.close)[0][0]
            std = self.am.std(self.boll_window)
            if std >= self.check_Ratio:
                self.start_indictor = True
                # boll = max(self.am.std(self.boll_window), self.check_Ratio * self.localPriceTick)
                self.boll_down, self.boll_up = self.boll_mid - std, self.boll_mid + std

                self.buy_price = round_to(self.boll_down, self.localPriceTick)
                self.sell_price = round_to(self.boll_mid, self.localPriceTick) - self.boll_plus * self.localPriceTick
                self.cover_price = round_to(self.boll_mid, self.localPriceTick) + self.boll_plus * self.localPriceTick
