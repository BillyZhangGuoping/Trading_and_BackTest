...
class AssistantStrategy(CtaTemplate):
    """"""
    ...
    def on_tick(self, tick: TickData):
        ...
        if tick.last_price >= self.high:
            if self.Trend == -1 and self.close_bar_count <=0 and self.Open_close !=-1:
                self.short(price=self.high, volume=self.fixed_size, stop=False)
                self.close_bar_count = 1
            if self.pos >0 and self.Open_close != 1:
                self.sell(price=self.high, volume=min(self.fixed_size, self.pos), stop=False)
                self.close_bar_count = 1
            ...
            if self.Trend == 1 and self.close_bar_count <=0 and self.Open_close !=-1:
                ...
                self.close_bar_count = 1
            if self.pos<0 and self.Open_close != 1:
                self.cover(price=self.low, volume=min(self.fixed_size, abs(self.pos)), stop=False)
                self.close_bar_count = 1
    ...
    # def trade_function(self):
    #     if self.close_bar_count <=0:
    #         if self.Open_close == 1:
    #             if self.Trend == 1:
    #                 self.buy(price = self.low - self.real_Markup,volume=self.fixed_size,stop=False)
    #             elif self.Trend == -1:
    #                 self.short(price = self.high +  self.real_Markup,volume=self.fixed_size,stop=False)
    #         elif self.Open_close == -1:
    #             if self.pos > 0:
    #                 self.sell(price = self.high + self.real_Markup,volume = min(self.fixed_size,self.pos),stop= False)
    #             elif self.pos < 0:
    #                 self.cover(price = self.low - self.real_Markup, volume=min(self.fixed_size,abs(self.pos)),stop = False)
