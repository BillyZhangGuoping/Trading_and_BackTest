...
2、所有品种按照不同板块录入数据库，选出一部分可以交易的活跃品种。类似母库与子库，只能交易子库的品种。
    未在策略踢醒
3、入场选择用30分钟K作为信号，然后均线作为参数，届时在GUI界面选择或者填都可以。
...
class OneMAMACDBarUpdatedStrategy(CtaTemplate):
	""""""
	...
	def on_bar(self, bar: BarData):
		...
		if self.pos == 0 :
			...
			self.price_rate = 0
			self.cancel_all()
			if bar.close_price > self.MA_value and self.MACD_BAR > sum(abs(hist[-2:-5])):
				self.buy(bar.close_price + 2, self.fixed_size, False)
			elif bar.close_price < self.MA_value and self.MACD_BAR <- sum(abs(hist[-2:-5])):
				self.short(bar.close_price - 2, self.fixed_size, False)

		elif self.pos > 0:
			if self.MACD_BAR <- sum(abs(hist[-2:-5])):
				self.cancel_all()
				...
				self.price_rate = (self.intra_trade_high - self.open_price) * 100.0 / self.open_price
			...
			if self.MACD_BAR <- sum(abs(hist[-2:-5])):
				...
				self.price_rate = (self.open_price - self.intra_trade_low) * 100.0 / self.open_price
