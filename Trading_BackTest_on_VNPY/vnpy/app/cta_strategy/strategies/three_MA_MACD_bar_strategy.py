...
class ThreeMAMACDBarStrategy(CtaTemplate):
	""""""
	...
	def on_bar(self, bar: BarData):
		...
		if self.pos == 0:
			...
			if bar.close_price > self.MA_value and self.MACD_RATE > self.OPEN_MACD_RATE and self.MACD_DEA > last_MACD_DEA and self.long_trend != -1:
				self.buy(bar.close_price + 2, self.fixed_size, False)
			elif bar.close_price < self.MA_value and self.MACD_RATE < -self.OPEN_MACD_RATE and self.MACD_DEA < last_MACD_DEA and self.long_trend != 1:
