...
class HighLowStrategy(CtaTemplate):
    """"""
    author = "Billy"
    init_pos = 0
    ...
    parameters = ["fixed_size", "bar_time", "bar_array_length", "Markup", "price_up", "price_down", "boll_window", "boll_dev", "isAG",
                  "lose_count_number","deal_open_count_number","std_limitation","tick_touch_count"]
    variables = ["real_Markup", "std", "high","low","trade_price", "close_long_price", "close_short_price", "lose_count","deal_open_count","priceArray_sum"]

    def __init__(self, cta_engine, strategy_name, vt_symbol, vt_local, setting):
        """"""
        super(HighLowStrategy, self).__init__(
    ...
    def on_tick(self, tick: TickData):
        ...
        elif self.pos < 0 and tick.last_price >= self.close_short_price:
    ...
    def on_min_bar(self, bar: BarData):
        ...
        # boll_up,boll_down = am.donchian(self.boll_window)
        self.start = False
        ...
        if self.check_trading_time(bar) and self.trading:
            ...
            if self.pos == 0 and self.lose_count <= 0:
                ...
                # self.send_open_long_deal(self.low)
                # self.send_open_short_deal(self.high)
