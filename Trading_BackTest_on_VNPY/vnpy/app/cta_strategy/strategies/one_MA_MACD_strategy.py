...
class OneMAMACDStrategy(CtaTemplate):
    """"""
    ...
    def on_tick(self, tick: TickData):
        ...
        """
        if tick.datetime.second in [10,20,30,40,50] and tick.datetime.microsecond == 0:
            local_close_array = copy(self.am.close_array)
            local_close_array[:-1] = local_close_array[1:]
            local_close_array[-1] = tick.last_price
            ...
            if self.pos == 0:
                if tick.last_price > self.MA_value and self.MACD_DIFF > self.MACD_DEA and self.long_trend != -1:
                    self.buy(tick.ask_price_1, self.fixed_size, False)
                elif tick.last_price < self.MA_value and self.MACD_DIFF < self.MACD_DEA and self.long_trend != 1:
                    self.short(tick.bid_price_1, self.fixed_size, False)
            elif self.pos > 0 and self.MACD_DIFF < self.MACD_DEA:
                    self.cancel_all()
                    self.sell(tick.bid_price_1,self.pos,False)

            elif self.pos > 0 and self.MACD_DIFF > self.MACD_DEA:
                    self.cancel_all()
                    self.cover(tick.bid_price_1,abs(self.pos),False)
    ...
    def on_bar(self, bar: BarData):
        ...
        if self.pos == 0:
            ...
            elif self.price_rate >= self.triger_profit_4 and self.price_rate < self.triger_profit_4 :
    ...
    def on_time_bar(self, bar: BarData):
        ...
        if self.pos == 0:
            if bar.close_price > self.MA_value and self.MACD_DIFF > self.MACD_DEA and self.long_trend != -1:
                self.buy(bar.close_price + 2, self.fixed_size, False)
            elif bar.close_price < self.MA_value and self.MACD_DIFF < self.MACD_DEA and self.long_trend != 1:
