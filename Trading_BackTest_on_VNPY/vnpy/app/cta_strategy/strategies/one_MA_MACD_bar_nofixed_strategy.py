...
class OneMAMACDBarNoFixedStrategy(CtaTemplate):
    """"""
    ...
    def on_bar(self, bar: BarData):
        ...
        if self.pos == 0:
            ...
            self.price_rate = 0
            self.cancel_all()
            if bar.close_price > MA_value_list[-1] and MA_value_list[-1] > MA_value_list[-2] and hist[-1]> hist[-2] and hist[-2] > hist[-3] and self.MACD_BAR > self.BAR_LIMIT \
                    and self.close_bar_count >= self.CLOSE_BAR and self.new_time_bar == True:
                self.buy(bar.close_price+1, self.fixed_size, True,lock=False,net=True)
            elif bar.close_price < MA_value_list[-1] and MA_value_list[-1] < MA_value_list[-2] and hist[-1]< hist[-2] and hist[-2] < hist[-3] and self.MACD_BAR < -self.BAR_LIMIT \
                    and self.close_bar_count >= self.CLOSE_BAR and self.new_time_bar == True:
                self.short(bar.close_price-1, self.fixed_size, True,lock=False,net=True)
    ...
    def on_trade(self, trade: TradeData):
        ...
        if trade.offset == Offset.CLOSE:
            if trade.direction == Direction.SHORT:
                lastDeal = (trade.price - self.entry_price)*100.0/ self.entry_price
                self.HeYueJiaZhi = self.HeYueJiaZhi + (trade.price - self.entry_price) * self.HeYueChengShu
            elif trade.direction == Direction.LONG:
                lastDeal = (self.entry_price - trade.price ) * 100.0 / self.entry_price
                self.HeYueJiaZhi = self.HeYueJiaZhi + (self.entry_price - trade.price ) *self.fixed_size*self.HeYueChengShu
            # print(self.HeYueJiaZhi)
            # self.fixed_size = int(self.HeYueJiaZhi/(self.HeYueChengShu*self.LastPrice*0.2))
            # self.fixed_size = 10
            if lastDeal > 2:
                # self.current_balRatio = 0.2
                self.i = []
                self.fixed_size = 2
            elif 0.5<lastDeal < 2:
                self.i.append(-2)
                self.fixed_size = max(2,self.fixed_size  - 2)
                # self.current_balRatio = max(0.2,self.current_balRatio-0.2)
            elif lastDeal < -0.5:
                # self.current_balRatio = min(self.current_balRatio + 0.2, 2)
                self.i.append(1)
                if sum(self.i[-8:])>=7:
                    self.fixed_size = min(self.fixed_size + 2 , 20)
                else:
                    self.fixed_size = min(10, self.fixed_size +1)
            #
            # self.i.append(self.current_balRatio)
            #
            # print(f" Counter {Counter(self.i)},")
            #
            # self.fixed_size = int((self.HeYueJiaZhi*self.current_balRatio) / (self.HeYueChengShu * self.LastPrice * 0.2))
        self.put_event()
