		...
		if self.interval == Interval.MINUTE:
			...
			if not (bar.datetime.minute + 1) % self.window:
				finished = True
					...
					and bar.exchange in [Exchange.SHFE, Exchange.DCE, Exchange.CZCE, Exchange.INE]
			):
				finished = True
...
class ArrayManager(object):
	...
	def kdj(self, fastk_period=9, slowk_period = 3, slowd_period=3, array: bool = False):
		kValue, dValue = talib.STOCH(self.high, self.low, self.close, fastk_period=9, slowk_period=3,
                               slowk_matype=0, slowd_period=3, slowd_matype=0)
		jValue = 3 * kValue - 2 * dValue

		if array:
			return kValue, dValue, jValue
		return kValue[-1], dValue[-1], jValue[-1]
