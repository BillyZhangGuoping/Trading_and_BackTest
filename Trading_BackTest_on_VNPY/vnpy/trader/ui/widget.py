...
class PositionMonitor(BaseMonitor):
    ...
    event_type_account = EVENT_ACCOUNT
    total_margin = 0
    data_key = "vt_positionid"
    sorting = True
    event_type_account_signal: QtCore.pyqtSignal = QtCore.pyqtSignal(Event)
    ...
    def register_event(self):
        ...
        if self.event_type_account:
            self.event_type_account_signal.connect(self.process_event)
            self.event_engine.register(self.event_type_account, self.event_type_account_signal.emit)
    ...
    def update_old_row(self, data: Any) -> None:
        ...
        if data.volume:
            ...
            remove_row = row_cells['volume'].row()
...
class AccountMonitor(BaseMonitor):
    ...
    headers = {
        ...
        "PositionProfit": {"display": "持仓盈亏", "cell": BaseCell, "update": True},
        "CurrentProfit": {"display": "当日盈亏", "cell": BaseCell, "update": True},
        "CurrMargin": {"display": "当前保证金", "cell": BaseCell, "update": True},
        "CurrMarginPrecent": {"display": "保证金比率", "cell": PercentCell, "update": True},
        # end Billy add
        "gateway_name": {"display": "接口", "cell": BaseCell, "update": False},
    }
    # Billy add
    def insert_new_row(self, data):
        ...
        Insert a new row at the top of table.
        """
        super(AccountMonitor, self).insert_new_row(data)
...
class TradingWidget(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        self.offset_combo = QtWidgets.QLineEdit("无开平 (净仓模式)")
        self.offset_combo.setReadOnly(True)

        # self.order_type_combo = QtWidgets.QComboBox()
        # self.order_type_combo.addItems(
        #     [order_type.value for order_type in OrderType])
        self.order_type_combo = QtWidgets.QLineEdit("限价 Limit")
        self.order_type_combo.setReadOnly(True)
    ...
    # 新增方法，更新offset_converter中order冻结信息
    ...
    def send_order(self) -> None:
        ...
        req_list = self.offset_converter.convert_order_request(original_req, lock = False, net = True)
